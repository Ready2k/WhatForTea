    ...
    <main className="max-w-lg mx-auto pb-8">
      ...
      <div className="px-4 pt-4 space-y-5">
        ...
        {recipe.nutrition_estimate && (() => {
          ...
          const isCard = n.source === 'card';
          const perLabel = `Per serving${n.per_servings ? ` (${n.per_servings})` : ''}`;

          // Macro totals for percentage bars (fat + carbs + protein)
          const macroTotal = (n.fat_g ?? 0) + (n.carbs_g ?? 0) + (n.protein_g ?? 0);
          const fatPct  = macroTotal > 0 ? Math.round(((n.fat_g  ?? 0) / macroTotal) * 100) : 0;
          const carbPct = macroTotal > 0 ? Math.round(((n.carbs_g ?? 0) / macroTotal) * 100) : 0;
          const protPct = macroTotal > 0 ? Math.round(((n.protein_g ?? 0) / macroTotal) * 100) : 0;

          const rows: Array<{ label: string; value: number | null | undefined; unit: string; sub?: boolean }> = [
            { label: 'Fat',             value: n.fat_g,        unit: 'g' },
            { label: 'of which Saturates', value: n.saturates_g, unit: 'g', sub: true },
            { label: 'Carbohydrates',   value: n.carbs_g,      unit: 'g' },
            { label: 'of which Sugars', value: n.sugars_g,     unit: 'g', sub: true },
            { label: 'Fibre',           value: n.fibre_g,      unit: 'g' },
            { label: 'Protein',         value: n.protein_g,    unit: 'g' },
            { label: 'Salt',            value: n.salt_g,       unit: 'g' },
          ];

          return (
            <div className="rounded-2xl border border-gray-100 dark:border-gray-700 overflow-hidden">
              {/* Header */}
              <div className="px-4 pt-4 pb-3 flex items-center justify-between bg-gray-50 dark:bg-gray-800/60">
                <div>
                  <h2 className="text-base font-semibold text-gray-900 dark:text-white">Nutrition</h2>
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{perLabel}</p>
                </div>
                <span
              ...
              {n.calories_kcal != null && (
                <div className="px-4 py-3 bg-white dark:bg-gray-900 flex items-baseline gap-2 border-b border-gray-100 dark:border-gray-700">
              ...
              {/* Macro bars */}
              {macroTotal > 0 && (
                <div className="px-4 py-3 bg-white dark:bg-gray-900 space-y-2 border-b border-gray-100 dark:border-gray-700">
                  <div className="flex h-2.5 rounded-full overflow-hidden gap-px">
                    {fatPct  > 0 && <div className="bg-rose-400 dark:bg-rose-500 transition-all"   style={{ width: `${fatPct}%` }} title={`Fat ${fatPct}%`} />}
                    {carbPct > 0 && <div className="bg-amber-400 dark:bg-amber-500 transition-all" style={{ width: `${carbPct}%` }} title={`Carbs ${carbPct}%`} />}
                    {protPct > 0 && <div className="bg-sky-400 dark:bg-sky-500 transition-all"     style={{ width: `${protPct}%` }} title={`Protein ${protPct}%`} />}
                  </div>
                  <div className="flex items-center gap-4 text-xs text-gray-500 dark:text-gray-400">
                    <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm bg-rose-400 dark:bg-rose-500 flex-shrink-0" />Fat</span>
                    <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm bg-amber-400 dark:bg-amber-500 flex-shrink-0" />Carbs</span>
                    <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm bg-sky-400 dark:bg-sky-500 flex-shrink-0" />Protein</span>
                  </div>
                </div>
              )}

              {/* Full nutrition table */}
              <div className="divide-y divide-gray-50 dark:divide-gray-700/60 bg-white dark:bg-gray-900 px-4">
                {rows.filter(r => r.value != null).map(({ label, value, unit, sub }) => (
                  <div key={label} className={`flex items-center justify-between py-2 ${sub ? 'pl-4' : ''}`}>
                    <span className={`text-sm ${sub ? 'text-gray-400 dark:text-gray-500' : 'text-gray-700 dark:text-gray-300'}`}>
                      {label}
                    </span>
                    <span className={`text-sm font-medium tabular-nums ${sub ? 'text-gray-500 dark:text-gray-400' : 'text-gray-900 dark:text-white'}`}>
                      {value}{unit}
                    </span>
                  </div>
                ))}
              ...
              {/* Disclaimer */}
              {!isCard && (
                <p className="px-4 py-2.5 text-xs text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-gray-800/60 border-t border-gray-100 dark:border-gray-700">
                  AI estimate from ingredients — not suitable for medical use
                </p>
              )}
            </div>
          );
